...
const AdjustPosition = () => {
  ...
  if (vault.toUpperCase() === TokenInfo.vaultAddress.toUpperCase()) {
    ...
    userTokenBalance = getBalanceAmount(quoteTokenValue?.symbol.toLowerCase() === 'wbnb' ? bnbBalance : tokenBalance)
    userQuoteTokenBalance = getBalanceAmount(
  ...
  const handleConfirm = async () => {
    ...
    if (vault.toUpperCase() === TokenInfo.vaultAddress.toUpperCase()) {
      ...
      if (Number(tokenInputValue) !== 0 && Number(quoteTokenInputValue) === 0) {
        ...
        farmingTokenAmount = getDecimalAmount(new BigNumber(quoteTokenInputValue || 0), 18)
          .toString()
          .replace(/\.(.*?\d*)/g, '') //  (quoteTokenInputValue)?.toString()
    ...
    if (
      data?.farmData?.lpSymbol.toUpperCase().includes('BNB') &&
      vault.toUpperCase() !== TokenInfo.vaultAddress.toUpperCase() &&
      wrapFlag &&
  ...
  const handleConfirmMinimizeAll = async () => {
    ...
    console.log({
      ...
      loan,
      convertedPositionValue,
      strategyWithdrawMinimizeTradingAddress,
      minfarmtokenValue,
      minfarmtoken,
      maxReturn,
      dataWorker,
  ...
  const handleSliderChange = (e) => {
    ...
    const finalValue =
      Number(value).toFixed(2) === Number(currentPositionLeverage).toFixed(2) ? currentPositionLeverage : Number(value)
      ...
      <Section>
        ...
        <Flex justifyContent="space-between">
          ...
          <Text bold>
            {isConvertTo ? (
              <>
                {new BigNumber(willReceive).toFixed(3, 1)} {tokenValueSymbol}
              </>
            ) : (
              <>
                {new BigNumber(willReceivefarm).toFixed(3, 1)} {quoteTokenValueSymbol} +{' '}
                {Number(willReceivebase).toPrecision(4)} {tokenValueSymbol}
              </>
            )}
          </Text>
        </Flex>
    ...
    lastSection = (
      <Section>
        ...
        <Flex justifyContent="space-between">
          ...
          {adjustData ? <Text bold>{new BigNumber(priceImpact * 100).toPrecision(3)}%</Text> : <Text bold>0.00%</Text>}
        </Flex>
        <Flex justifyContent="space-between">
          <Flex>
            <Text>{t('Trading Fees')}</Text>
          ...
          {adjustData ? (
            <Text bold>{new BigNumber(tradingFees * 100).toPrecision(3)}%</Text>
          ) : (
  ...
  const datalistOptions = (() => {
    for (let i = 1; i < (leverage < currentPositionLeverage ? currentPositionLeverage : leverage) / 0.5; i++) {
  ...
  useEffect(() => {
    if (leverageAfter) setTargetPositionLeverage(Number(leverageAfter))
  }, [leverageAfter])
